...
def open_zarr(year, month, datestart, dateend):
    ...
    s3map = s3fs.S3Map(datestring + "eastward_wind_at_100_metres.zarr/", s3=fs)
    precip_zarr = xr.open_dataset(s3map, engine="zarr")
    print(precip_zarr.info())
    precip_zarr = precip_zarr.sel(time0=slice(np.datetime64(datestart), np.datetime64(dateend)))
    return precip_zarr.eastward_wind_at_100_metres
